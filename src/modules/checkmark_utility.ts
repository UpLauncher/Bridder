(function () {
  ...
  const whiteCheckMark = () => {
    ...
    if (body && body.style.backgroundColor != "rgb(255, 255, 255)") {
  ...
  const verifiedBlue = () => {
    document.querySelectorAll("svg.r-yyyyoo").forEach((container) => {
      if (container.querySelector("linearGradient")) return;
    ...
    document.querySelectorAll("div").forEach((container) => {
      if (
        ...
        if (container.parentNode) {
          (container.parentNode as HTMLElement).style.display = "none";
        }
      }
    });
    ...
    .then((result) => {
      if (result.enabled) {
        if (result.verifiedBlue) {
          new MutationObserver(verifiedBlue).observe(document.body, {
            childList: true,
            subtree: true,
          });
        ...
        if (result.blue_delete) {
          new MutationObserver(blueDelete).observe(document.body, {
            childList: true,
            subtree: true,
          });
        ...
        if (result.white_checkmark) {
          new MutationObserver(whiteCheckMark).observe(document.body, {
            childList: true,
            subtree: true,
          });
