export interface Setting {
  ...
  category: string;
  description: string;
  default: boolean; // デフォルト値の型を指定
}

export function applyDefaultSettings() {
  const defaultSettings: Record<string, boolean> = {}; // defaultの型をbooleanに変更

  settingsData.forEach((setting: Setting) => {
    defaultSettings[setting.id] = setting.default; // 各設定のデフォルト値をオブジェクトに追加
  });

  defaultSettings["enabled"] = true;

  // chrome.storage.sync にデフォルト値を保存
  chrome.storage.sync.set(defaultSettings, () => {
    console.log('すべての設定がデフォルトに設定されました');
  });
}
...
export const settingsData: Setting[] = [
    ...
    { id: 'icon_mode', name: 'アイコンの置き換え', type: 'checkbox', description: 'XのロゴをTwitterの青い鳥に変更します。', category: '表示', default: true },
    ...
    { id: 'white_checkmark', name: '白いチェックマークを有効にする', type: 'checkbox', description: 'チェックマークを白色に変更します。(ダークテーマのみ)', category: 'チェックマーク', default: true },
    ...
    { id: 'tcoBypasser', name: 't.coをバイパスする', type: 'checkbox', description: 'Twitterが改悪をしているt.coをバイパスすることができます。(一部のみ、バグが発生する可能性が大)', category: 'その他', default: false }
  ];
